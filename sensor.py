...
def _check_domain_whitelisted(query):
    ...
    if result is None:
        # 域名  以r"(?i)[^A-Z0-9._-]分割  判断域名是否在WHITELIST里面
    ...
    try:
        ...
        if protocol == socket.IPPROTO_TCP:  # TCP
            ...
            if flags == 2:  # SYN set (only)
                ...
                if method and path:
                    # method 是 GET
                    # path是'/latest/meta-data/region-id'
                    post_data = None
                    host = dst_ip
                    # 查找到 host的索引
                    first_index = tcp_data.find("\r\nHost:")
                    ...
                    # path路径中"probe", "proxy", "echo", "check" 满足端口80 http打头   域名不在白名单内
                    if config.USE_HEURISTICS and dst_port == 80 and path.startswith("http://") and any(_ in path for _ in SUSPICIOUS_PROXY_PROBE_PRE_CONDITION) and not _check_domain_whitelisted(path.split('/')[2]):
                        # 以 / 开头 并且只能出现1 次  http://latest/meta-data/proxy变成了http://latest/(meta-data/proxy)
                    ...
                    if config.USE_HEURISTICS:
                        ...
                        if user_agent:
                            ...
                            if result is None:
                                ...
                                if not result:
                                    # <core.datatype.LRUDict object at 0x7f0cc0ad1160>
                    ...
                    # _check_domain_whitelisted检查白名单域，如果不是在白名单的域
                    if not _check_domain_whitelisted(host):
                        # 分析网页，获取路径
                        unquoted_path = _urllib.parse.unquote(path)
                        unquoted_post_data = _urllib.parse.unquote(post_data or "")
                        # unquoted_post_data解析为status=False&msg=请先登陆宝塔官网用户

                        # 返回删除 checks字符串 末尾的指定字符/  后生成的新字符串。获得/之前的字符串
                        checks = [path.rstrip('/')]
                        # 这里的checks ['/latest/meta-data/region-id']
                        if '?' in path:
                            ...
                            # ['/api/panel/get_soft_list_test', '/api/panel/get_soft_list_test?status=false&msg=请先登陆宝塔官网用户']
